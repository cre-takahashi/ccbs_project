...
async function finddata(req, res) {
  ...
  var happyoSuData = []
  var bccoin = 0
  getCoinDatasAll = await findGetCoinAll(req, 0)
  getCoinDatas = await findGetCoin(req, 0)
  takeCoinDatasAll = await findTakeCoinAll(req, 0)
  takeCoinDatas = await findTakeCoin(req, 0)
  shainDatas = await findTShain(req)
  nendoDatas = await findNendo(req)
  if (req.body.kengenCd != '1') {
...
function findGetCoinAll(req, shoriId) {
  return new Promise((resolve, reject) => {
    ...
    if (shoriId == 0) {
      if (req.body.kengenCd == '1') {
        var sql = findGetCoinAllSql + 'order by tzo.insert_tm'
        db
          .query(sql, {
        ...
        var sql =
          findGetCoinAllSql +
          'and tsha.t_shain_pk = :mypk order by tzo.insert_tm'
        db
          .query(sql, {
      ...
      if (req.body.kengenCd == '1') {
        if (nendo != '' && manager != '') {
          ...
          var sql =
            findGetCoinAllSql +
            "and to_char(tzo.insert_tm,'yyyy/mm/dd') >= :nendoStart and :nendoEnd >= to_char(tzo.insert_tm, 'yyyy/mm/dd') and tsha.shimei = :maneger order by tzo.insert_tm"
          db
            .query(sql, {
              replacements: {
          ...
          var sql =
            findGetCoinAllSql +
            "and to_char(tzo.insert_tm,'yyyy/mm/dd') >= :nendoStart and :nendoEnd >= to_char(tzo.insert_tm, 'yyyy/mm/dd') order by tzo.insert_tm"
          db
            .query(sql, {
              replacements: {
                nendoStart: nendoStart,
          ...
          var sql =
            findGetCoinAllSql +
            'and tsha.shimei = :maneger order by tzo.insert_tm'
          db
            .query(sql, {
        ...
        if (nendo != '') {
          ...
          var sql =
            findGetCoinAllSql +
            "and tsha.t_shain_pk = :mypk and to_char(tzo.insert_tm,'yyyy/mm/dd') >= :nendoStart and :nendoEnd >= to_char(tzo.insert_tm, 'yyyy/mm/dd') order by tzo.insert_tm"
          db
            .query(sql, {
              replacements: {
...
function findGetCoin(req, shoriId) {
  return new Promise((resolve, reject) => {
    ...
    if (shoriId == 0) {
      console.log('初期処理実行')
      if (req.body.kengenCd == '1') {
        var sql = findGetCoinSql + 'order by tzo.insert_tm'
        db
          .query(sql, {
            type: db.QueryTypes.RAW
          })
        ...
        var sql =
          findGetCoinSql + 'and tsha.t_shain_pk = :mypk order by tzo.insert_tm'
        db
          .query(sql, {
            replacements: { mypk: req.body.tShainPk },
            type: db.QueryTypes.RAW
      ...
      if (req.body.kengenCd == '1') {
        if (nendo != '' && manager != '') {
          ...
          var sql =
            findGetCoinSql + 'and tsha.shimei = :maneger order by tzo.insert_tm'
          db
            .query(sql, {
              replacements: { maneger: manager },
              type: db.QueryTypes.RAW
...
function findTakeCoinAll(req, shoriId) {
  return new Promise((resolve, reject) => {
    if (shoriId == 0) {
      if (req.body.kengenCd == '1') {
        var sql = findTakeCoinAllSql + 'order by tzo.insert_tm'
        db
          .query(sql, {
            type: db.QueryTypes.RAW
          })
...
function findTakeCoin(req, shoriId) {
  return new Promise((resolve, reject) => {
    if (shoriId == 0) {
      if (req.body.kengenCd == '1') {
        var sql = findTakeCoinSql + 'order by tzo.insert_tm'
        db
          .query(sql, {
            type: db.QueryTypes.RAW
          })
        ...
        var sql =
          findTakeCoinSql + 'and tsha.t_shain_pk = :mypk order by tzo.insert_tm'
        db
          .query(sql, {
            replacements: { mypk: req.body.tShainPk },
            type: db.QueryTypes.RAW
...
function countHappyoSu(req, shoriId) {
  return new Promise((resolve, reject) => {
    if (shoriId == 0) {
      ...
      if (nendo != '') {
        var sql =
          countHappyoSuSql + "and to_char(tpre.insert_tm,'yyyy') = :nendo"
        db
          .query(sql, {
            replacements: {
              mypk: req.body.tShainPk,
...
function findNendo(req) {
  return new Promise((resolve, reject) => {
    var sql =
      "select to_char(tzo.insert_tm,'yyyy') as year" +
      ' from t_zoyo tzo' +
      " where tzo.delete_flg = '0'" +
      " group by to_char(tzo.insert_tm,'yyyy')"
    db
      .query(sql, {
        type: db.QueryTypes.RAW
      })
